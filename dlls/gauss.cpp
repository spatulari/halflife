	...
	while (flDamage > 10 && nMaxHits > 0)
	{
		...
		if ( pEntity->ReflectGauss() )
		{
			...
			if (n < 0.5) // 60 degrees
			{
				...
				// explode a bit
				m_pPlayer->RadiusDamage( tr.vecEndPos, pev, m_pPlayer->pev, flDamage * n, EntityClass::None, DMG_BLAST );
				...
				if ( !m_fPrimaryFire )
				{
					...
					if (!beam_tr.fAllSolid)
					{
						...
						if (n < flDamage)
						{
							...
							// exit blast damage
							//m_pPlayer->RadiusDamage( beam_tr.vecEndPos + vecDir * 8, pev, m_pPlayer->pev, flDamage, EntityClass::None, DMG_BLAST );
							float damage_radius;
							...
							::RadiusDamage( beam_tr.vecEndPos + vecDir * 8, pev, m_pPlayer->pev, flDamage, damage_radius, EntityClass::None, DMG_BLAST );
