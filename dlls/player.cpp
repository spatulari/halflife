...
void CBasePlayer::SetAnimation( PLAYER_ANIM playerAnim )
{
	...
	default:
		...
		//ALERT(AlertType::Console, "Set die animation to %d\n", animDesired);
...
void CBasePlayer :: UpdatePlayerSound ( void )
{
	...
	if ( !pSound )
	{
		ALERT ( AlertType::Console, "Client lost reserved sound!\n" );
		return;
	}
...
// fallpain sound, and damage will be inflicted based on how far the player fell
	...
	{		
		if (m_flFallVelocity > 64 && !g_pGameRules->IsMultiplayer())
		{
			...
			// ALERT( AlertType::Console, "fall %f\n", m_flFallVelocity );
		}
		m_flFallVelocity = 0;
	}
...
// choose a info_player_deathmatch point
	...
	else if ( g_pGameRules->IsDeathmatch() )
	{
		...
		if ( FNullEnt( pSpot ) )  // skip over the nullptr point
			pSpot = UTIL_FindEntityByClassname( pSpot, "info_player_deathmatch" );
...
// dont let uninitialized value here hurt the player
	...
	{
		ALERT ( AlertType::Console, "Couldn't alloc player sound slot!\n" );
	}
...
#define clamp( val, min, max ) ( ((val) > (max)) ? (max) : ( ((val) < (min)) ? (min) : (val) ) )
	...
	//
	// for each weapon:
	// byte		name str length (not including nullptr)
	// bytes... name
	// byte		Ammo Type
	// byte		Ammo2 Type
	// byte		bucket
...
void CBasePlayer::DropPlayerItem ( char *pszItemName )
{
	...
	if ( !strlen( pszItemName ) )
	{
		...
		// make the string nullptr to make future operations in this function easier
		pszItemName = nullptr;
	} 
