...
void PM_InitTextureTypes()
{
	...
	while ( pmove->memfgets( pMemFile, fileSize, &filePos, buffer, 511 ) != nullptr && (gcTextures < CTEXTURESMAX) )
	{
		...
		// nullptr-terminate name and save in sentences array
...
int PM_CheckStuck (void)
{
	...
	VectorAdd(base, offset, test);
	if ( ( hitent = pmove->PM_TestPlayerPositionEx ( test, nullptr, PM_Ignore ) ) == -1 )
	{
		//Con_DPrintf("Nudged\n");
		...
		for ( z = 0; z <= zminmax; z += zstep )
		{
			for ( x = -xyminmax; x <= xyminmax; x += xystep )
			{
				for ( y = -xyminmax; y <= xyminmax; y += xystep )
				{
					...
					if ( pmove->PM_TestPlayerPositionEx ( test, nullptr, PM_Ignore ) == -1 )
					{
						VectorCopy( test, pmove->origin );
						return 0;
					}
...
void PM_FixPlayerCrouchStuck( int direction )
{
	...
	hitent = pmove->PM_TestPlayerPositionEx ( pmove->origin, nullptr, PM_Ignore );
	if (hitent == -1 )
		return;
	...
	if ( trace.fraction != 1.0 )
	{
		...
		else
		{
			if ( forward != 0 || right != 0 )
			{
				...
				//ALERT(AlertType::Console, "pev %.2f %.2f %.2f - ",
